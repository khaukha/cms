...
<%
	...
	List<WnImXpEntity> wns = task.getUnConsolidatedWns(wnc);
	double total_tons = 0;
	int total_bags = 0;
	for (int i = 0; i < wns.size(); i++) {
		WnImXpEntity wn = wns.get(i);
		WnConsolidationEntity _wnc = wn.getConsolidation().getWnConsolidation();
...
%>
	...
	<tr bgcolor="<%=bgcolor%>" onClick="highlightOn(this)">
	  <td width="42" align="right" bgcolor="#DDDDDD"><%=wn.getIdLong()%>&nbsp;</td>
		...
		<td width="50" align="right"><%=Numeric.numberToStr(wn.stock_bags,0)%>&nbsp;</td>
		<td width="60" align="right"><%=Numeric.numberToStr(wn.stock_weight/1000,4)%>&nbsp;</td>
