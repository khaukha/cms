...
<%
	...
	List<WnrAllocationEntity> wnas = wnt.getWnrAllocations();
	int no = 1;
	...
	for (int i = 0; i < wnas.size(); i++) {
		...
		sum.add(wna);
		String wnr_color = Action.getColor(wna.status);//wna.status > 0 ? Action.getColor(2) : Action.getColor(1); 
...
%>				  
		  ...
		  <input type="hidden" name="pallet_id_<%=no%>"  id="pallet_id_<%=no%>" value="<%=wne.pallet_id%>">
                  <tr id="wnr_<%=no%>" class="style11" onClick="highlightOn(this);">
                    ...
                    <td align="center"><input type="text" name="wnr_date_out_<%=no%>" id="wnr_date_out_<%=no%>" class="style11" style="width:60px; text-align:center" value="<%=DateTime.dateToStr(wna.date_out)%>" readonly></td>
					...
					<td align="center"><input type="checkbox" name="checked_<%=no%>" id="checked_<%=no%>" value="2" border="0" <%=wna.status==2?"checked":""%> onClick="pass_RW(this,<%=no%>)" <%=task.isReadOnly()?"disabled":""%>></td>
                    <td align="center">&nbsp;</td>
                  </tr>
<%
		no++;
