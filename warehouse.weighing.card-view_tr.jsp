<%
	CheckingTrucking checking = task.getCheckingTrucking();	
	WnAllocationEntity wnt = checking.doTask();
	wn = wnt.getWeightNote();
	if (wnt.getIdLong() != 0) {
		//sc.inst_id = wnt.inst_id;
	}
	...
	wnr = checking.doCheckingWnr(wnt);
	auto_weighing = false;
%>
          ...
          <tr>
            <td valign="top"><table width="100%"  border="0" cellpadding="0" cellspacing="1" class="style11">
              <tr bgcolor="#EEEEEE">
                ...
                <th width="135px"><input name="search_inst" id="search_inst" type="text"  class="style11" style="width:150;" value="<%=search_inst%>" onKeyUp="doSearch1(event)"></th>
                <th width="135px">From</th>
                <th width="135px">To</th>
                <th>Grade</th>
                </tr>
              <tr>
                ...
                <td><select name="filter_inst_id" id="filter_inst_id" size=7 class="style11" style="width:100%;" onChange="setValue('wn_id',0);itemSelected(this);"><%=Html.selectOptionsX(checking.getInstList(),sc.filter_inst_id,"All")%></select></td>
                ...
                <td><select name="filter_grade_id" id="filter_grade_id" size=7 class="style11" style="width:100%;" onChange="doPost();"><%=Html.selectOptions(dao.getTruckingDao().getGradeFilter(),sc.filter_grade_id,"All")%></select></td>
                </tr>
            </table></td>
          </tr>
          <tr>
            <td><table width="100%"  border="1" cellpadding="0" cellspacing="0" class="style2">
              <tr>
                <td><table id="main_frame_" width="100%"  border="0" cellspacing="0" cellpadding="0">
                          ...
                          <tr>
                            <td width="42%" valign="top"><table width="100%"  border="0" cellpadding="1" cellspacing="0" class="style2">
                              <tr>
                                ...
                                <td style="color:<%=Action.getColor(wn.status)%>"><strong><%=wnt.getWeightNote().getRefNumber()%></strong>&nbsp;&nbsp;<%=DateTime.dateToStr(wn.date)%></td>
                              </tr>
                              <tr>
		                        <td align="right">Contract Ref &nbsp;</td>
        		                <td><strong><%=wn.getInstruction().getRefNumber()%></strong></td>
                              </tr>
                            ...
                            <td valign="top"><table width="100%"  border="0" cellpadding="1" cellspacing="0" class="style2">
                              <tr>
        		                ...
        		                <td style="color:<%=Action.getColor(wnt.getWeightNote().getQualityReport().status)%>"><strong><%=wnt.getWeightNote().getQualityReport().getRefNumber()%></strong></td>
                              </tr>
                              <tr>
		                        <td align="right">Grade &nbsp;</td>
        		                <td><strong><%=wnt.getWeightNote().getGrade().short_name%></strong></td>
                              </tr>
                            </table></td>
                          </tr>
</table></td>
                  ...
                  <tr>
                    <td><%@include file="warehouse.weighing.card-view.wnr-tr.jsp"%></td>
                  </tr>
                  <tr>
                    <td><table width="100%"  border="0" cellpadding="0" cellspacing="1" class="style11">
                  <tr style="display:">
                    <td width="150px" align="right">Completed &nbsp;</td>
                    <td width="24px"><%=Html.checkBox("status", wnt.status)%></td>
					<td width="94"><%=Html.datePicker("date", wnt.date, "style11")%></td>
                    <td align="right">Updated By <%=wnt.getUser().user_name%> &nbsp;</td>
                  </tr>
                </table></td>
                  </tr>
                </table></td>
...
<script language="javascript">
	getObj("search_key").title = "Input a number to search Trucking.";
</script>