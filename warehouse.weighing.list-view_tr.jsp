...
<table width="100%"  border="0" cellpadding="0" cellspacing="1" class="style11">
          ...
          <tr>
            ...
            <td><select name="filter_status" id="filter_status" size=7 class="style11" style="width:100%;" onChange="doPost();"><%=Html.selectOptions(dao.getStatusDao().selectAll(),sc.filter_status,"All")%></select></td>
             <td><select name="filter_source_id" id="filter_source_id" size=7 class="style11" style="width:100%;" onChange="doPost();"><%=Html.selectOptions(dao.getWarehouseDao().selectByLocationId(location_id),sc.filter_source_id,"All")%></select></td>
...
<%
	...
	for (int i = 0; i < wns.size(); i++) {
		...
		TruckingEntity tr = (TruckingEntity)(wn.getInstruction());		
%>				
                <tr onClick="highlightOn(this);" onDblClick="wnDblClicked(this,<%=wn.getIdLong()%>,<%=wn.inst_id%>)">
                  ...
                  <td width="100"><%=tr.getRefNumber()%></td>
                  ...
                  <td width="60" align="right"><%=Numeric.numberToStr(wn.allocated_weight/1000,3)%></td>
                  <td width="60" align="right"><%=Numeric.numberToStr(wn.weight_out/1000,3)%></td>
                  <td width="60" align="right"><%=Numeric.numberToStr(wn.weight_loss/1000,3)%></td>
                  <td width="60" align="right"><%=Numeric.numberToStr(wn.getPendingWeight()/1000,3)%></td>
				  <td>&nbsp;</td>
                </tr>
<%
	}
...
<table width="100%"  border="0" cellpadding="0" cellspacing="1" class="style11">
          <tr style="font-weight:bold" align="right" bgcolor="#DDDDDD">
		  	<td width="42">&nbsp;</td>
            <td width="838" align="center">Total</td>
            <td width="60"><%=Numeric.numberToStr(sum.allocated_bags,0)%></td>
            <td width="60"><%=Numeric.numberToStr(sum.allocated_weight/1000,3)%></td>
            <td width="60"><%=Numeric.numberToStr(sum.weight_out/1000,3)%></td>
            <td width="60"><%=Numeric.numberToStr(sum.weight_loss/1000,3)%></td>
            <td width="60"><%=Numeric.numberToStr(pending_weight/1000,3)%></td>
			<td width="">&nbsp;</td>
          </tr>
</table>
